...
class TaskMapper
{
    ...
    {
        try {
            $sql = 'SELECT `userid`, `username`, `e-mail`, `text`, `fulfilled` FROM `exercice` WHERE `id` = :id';
    ...
    function addTask(UserMapper $userMapper, $userid, $email, $text)
    {
        try {
            ...
            $sql = 'INSERT INTO `exercice`(`userid`, `username`, `e-mail`, `text`, `fulfilled`)
    ...
    {
        try {
            $sql = 'UPDATE `exercice` SET `text` = :text WHERE `id` = :id';
            $stmt = $this->pdo->prepare($sql);
